...
//	$uddsql="SELECT count(a.id) FROM `#__uddeim` AS a LEFT JOIN `#__users` AS b ON a.fromid=b.id WHERE a.totrash=0 AND a.toread=0 AND a.toid=".(int)$udduserid;
	...
	if ($uddresult>0) {
        ...
        if ($uddshowicons)
        $uddout .= '<img src="components/com_uddeim/templates/'.$uddconfig->templatedir.'/images/delayed_im.gif" alt="new" title="'._UDDEMODULE_NEWMESSAGES.'">&nbsp;';
		$uddout .= '<span>'._UDDEMODULE_NEWMESSAGES.': <b>'.$uddresult.'</b></span>'; //_UDDEMODULE_NEWMESSAGES." ".$uddresult;
		$uddout .= "</p>";
	}
}

$iconout = "<img src='".$uddpathtosite."/components/com_uddeim/templates/".$uddconfig->templatedir."/images/menu_outbox.gif' alt='"._UDDEIM_OUTBOX."' />";
$iconin = "<img src='".$uddpathtosite."/components/com_uddeim/templates/".$uddconfig->templatedir."/images/menu_inbox.gif' alt='"._UDDEIM_INBOX."' />";
