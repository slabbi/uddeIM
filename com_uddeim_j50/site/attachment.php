	...
	if ($config->enableattachment) {
		...
		foreach ($tmp_files["tmp_name"] as $key => $value) {
			...
			if ( $tmp_name && $block ) {
                ...
                $uddeimfiles = '/images/uddeimfiles';
                if (!uddeIMfolderExists($uddeimfiles)) {
		        if (!uddeIMmkdir($uddeimfiles))
			    echo "<b><span style='color: red;'>"._UDDEADM_FOLDERCREATE_ERROR.$uddeimfiles."</span></b>";
                else
                chmod($uddeimfiles,0766);
                }
				$uploaddir  = uddeIMgetPath('absolute_path').$uddeimfiles;
				$id = md5(rand() * time());
