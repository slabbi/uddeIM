...
// @description   Instant Messages System for Joomla 5, admin shared
...
$versionstring	= "uddeIM 5.3";						// version string for about boxes
$checkversion	= "5.3";							// version as above for check for update - this is the version we have
$checkhotfix	= "3";								// version as above for check for update - this is the version we have
...
function uddeIMcheckJversion() {					// borrowed from Cummunity Builder, -1 = Mambo>=4.6, 0 = Mambo <=4.5/J1.0, 1 = J1.5
    ...
    $shortVersion = $version->getShortVersion();
		if (!strncasecmp($shortVersion, "3.0", 3)) {
			$ver = 5;
...
function uddeIMcheckKU() {
    $k1 = Joomla\CMS\Component\ComponentHelper::isEnabled ( 'com_kunena', true );
	$k2 = class_exists('Kunena\Forum\Libraries\Forum\KunenaForum') && Kunena\Forum\Libraries\Forum\KunenaForum::isCompatible('6.0') && Kunena\Forum\Libraries\Forum\KunenaForum::installed();
    return $k1 || $k2;
}
...
function uddeIMdoShowAllUsers($myself, $my_gid, $config, $mode, $enabled=1, $defaultvalue=0) {
	...
	if (uddeIMcheckJversion()>=2) {		// greater J1.6
		$hide = "";
		if ($config->hideusers)
	...
	} elseif ($mode ==3) {  // AUTOCOMPLETE List
        $allnames = json_encode($rows,JSON_PARTIAL_OUTPUT_ON_ERROR);
