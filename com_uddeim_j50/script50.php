...
// @description   Instant Messages System for Joomla 5, install script
...
class com_uddeimInstallerScript extends \stdClass
{
    public $release;
    public $minimum_joomla_release;
    ...
    function postflight($type, $parent) {
        ...
        if ( $type=='update' ){
        echo '<p>' . Text::_('COM_UDDEIM_BAKUP_TEXT') . '</p>';
...
function uddeIM_com_install() {
    ...
    $adminpath = uddeIMgetPath('absolute_path')."/administrator/components/com_uddeim/";

	require_once($adminpath."config.class.php");
	require_once($adminpath."admin.shared.php");
	require_once($adminpath."admin.includes.php");
    ...
    $suser = Factory::getApplication()->getIdentity()->id; 
	$config->hideusers = $suser;
	$config->pubhideusers = $suser;
